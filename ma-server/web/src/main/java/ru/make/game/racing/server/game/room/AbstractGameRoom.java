...
public abstract class AbstractGameRoom implements GameRoom, WebsocketHandler {
    ...
    protected AbstractGameRoom(UUID gameRoomId,
                               GameRoomManagementService gameRoomManagementService,
    ...
    public void start(long startDelay, long endDelay, long loopRate) {
        futureList.addAll(Arrays.asList(getRoomExecutorService().scheduleAtFixedRate(this,
                startDelay, loopRate, TimeUnit.MILLISECONDS), getRoomExecutorService().schedule(() -> {
            onBattleEnded();
